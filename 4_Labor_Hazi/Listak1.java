...
class Listak1 {
    ...
    private static int matchEnds(List<String> words) {
        ...
        for (String word : words) {
            if (word.length() >= 2) {
                char elso_betu = word.substring(0).charAt(0);
                char utolso_betu = word.substring(word.length() - 1).charAt(0);

                if (elso_betu == utolso_betu) {
                    count++;
                }
            }
        }
    ...
    private static List<String> frontX(List<String> words) {
        List<String> x_lista = new ArrayList<String>();
        List<String> Lista = new ArrayList<String>();
        List<String> Kiegiszito_Lista = new ArrayList<String>();
        ...
        x_lista.sort(null);
        Lista.sort(null);
        Kiegiszito_Lista.addAll(x_lista);
        Kiegiszito_Lista.addAll(Lista);

        return Kiegiszito_Lista;
